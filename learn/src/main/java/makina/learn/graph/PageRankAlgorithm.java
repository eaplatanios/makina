...
public class PageRankAlgorithm<E> {
    ...
    private PageRankAlgorithm(Builder<E> builder) {
        ...
        dampingTerm = (1.0 - dampingFactor) / graph.numberOfVertices();
    ...
    private VertexContentType vertexComputeFunction(Vertex<VertexContentType, E> vertex) {
        double oldRank = vertex.content().rank;
        double newRank = 0.0;
        for (Edge<VertexContentType, E> incomingEdge : vertex.incomingEdges())
            newRank += incomingEdge.sourceVertex().content().rank / incomingEdge.sourceVertex().numberOfOutgoingEdges();
