...
public class HITSAlgorithm<E> {
    ...
    private void performIterationUpdates() {
        ...
        if (checkForScoresConvergence) {
            ...
            for (Vertex<VertexContentType, E> vertex : graph.vertices())
                lastMeanAbsoluteRankChange += vertex.content().lastAbsoluteChange;
    ...
    private VertexContentType vertexComputeFunction(Vertex<VertexContentType, E> vertex) {
        double oldAuthorityScore = vertex.content().authorityScore;
        double oldHubScore = vertex.content().hubScore;
        double newAuthorityScore = 0.0;
        double newHubScore = 0.0;
        for (Edge<VertexContentType, E> incomingEdge : vertex.incomingEdges())
            newAuthorityScore += incomingEdge.sourceVertex().content().hubScore;
        for (Edge<VertexContentType, E> outgoingEdge : vertex.outgoingEdges())
            newHubScore += outgoingEdge.destinationVertex().content().authorityScore;
        authorityNormalizationConstant += newAuthorityScore * newAuthorityScore;
        hubNormalizationConstant += newHubScore * newHubScore;
        return new VertexContentType(vertex.content().id,
                                     newAuthorityScore,
                                     newHubScore,
