...
import makina.learn.data.FeatureMap;
import makina.learn.data.FeatureMapMySQL;
...
public class FeaturesPreprocessing {
    ...
    private static void parseVariasCategoriesFiles() {
        if (Files.exists(Paths.get(labeledDataDirectory))) {
            labeledData = Utilities.readMap(labeledDataDirectory);
            ...
            try {
                ...
                Utilities.writeMap(labeledData, labeledDataDirectory);
    ...
    public static FeatureMap<SparseVector> buildCPLFeatureMap(FeatureMap<SparseVector> featureMap) {
    ...
    public static Map<String, Integer> buildCPLContextsMap() throws IOException {
    ...
    private static void buildADJFeatureMap(FeatureMapMySQL<SparseVector> featureMap) {
        ...
        try {
            if (Files.exists(Paths.get(adjFeatureMapDirectory + "/adjectives.bin"))) {
                adjectives = Utilities.readMap(adjFeatureMapDirectory + "/adjectives.bin");
            } else {
                ...
                Utilities.writeMap(adjectives, adjFeatureMapDirectory + "/adjectives.bin");
            }
            Stream<String> npAdjectivePairsLines = new BufferedReader(new InputStreamReader(
    ...
    public static void filterLabeledDataByFeatureMap(FeatureMapMySQL<SparseVector> featureMap) {
        if (Files.exists(Paths.get(filteredLabeledDataDirectory))) {
            filteredLabeledData = Utilities.readMap(filteredLabeledDataDirectory);
            ...
            Utilities.writeMap(filteredLabeledData, filteredLabeledDataDirectory);
