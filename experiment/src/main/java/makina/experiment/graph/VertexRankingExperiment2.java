...
public class VertexRankingExperiment2 {
    ...
    private final Map<Integer, Vertex<DeepGraph.VertexContent, Void>> trainingRNNVertexIndexesMap = new HashMap<>();
    private final Map<Integer, Vertex<DeepGraph.VertexContent, Void>> testingRNNVertexIndexesMap = new HashMap<>();
    ...
    private final Graph<DeepGraph.VertexContent, Void> trainingRNNGraph;
    private final Graph<DeepGraph.VertexContent, Void> testingRNNGraph;
    ...
    public VertexRankingExperiment2(DataSets.DataSet trainingDataSet,
        ...
        for (int vertexIndex : trainingDataSet.getVertexIndices()) {
            ...
            Vertex<DeepGraph.VertexContent, Void> rnnGraphVertex =
                    new Vertex<>(new DeepGraph.VertexContent(vertexIndex, 0, null, null));
        ...
        switch (rankingAlgorithm) {
            case PAGERANK:
                ...
                for (Vertex<PageRankAlgorithm.VertexContentType, Void> vertex : trainingPRGraph.vertices())
                    trueTrainingScores.put(vertex.content().getId(), Vectors.dense((double) vertex.content().getRank()));
                ...
                for (Vertex<PageRankAlgorithm.VertexContentType, Void> vertex : testingPRGraph.vertices())
                    trueTestingScores.put(vertex.content().getId(), Vectors.dense((double) vertex.content().getRank()));
                break;
            case HITS:
                ...
                for (Vertex<HITSAlgorithm.VertexContentType, Void> vertex : trainingHITSGraph.vertices())
                    trueTrainingScores.put(vertex.content().getId(), Vectors.dense(vertex.content().getAuthorityScore(),
                                                                                   vertex.content().getHubScore()));
                ...
                for (Vertex<HITSAlgorithm.VertexContentType, Void> vertex : testingHITSGraph.vertices())
                    trueTestingScores.put(vertex.content().getId(), Vectors.dense(vertex.content().getAuthorityScore(),
                                                                                  vertex.content().getHubScore()));
                break;
        }
    }
    ...
    private EvaluationResults runRNNExperiment(int featureVectorsSize, int maximumNumberOfSteps) {
        ...
        if (rankingAlgorithm.equals(RankingAlgorithm.HITS))
            outputSize = 2;
        VertexRankingDeepGraph<Void> trainGRNN =
                new VertexRankingDeepGraph<>(featureVectorsSize, outputSize, maximumNumberOfSteps, trainingRNNGraph, updateFunctionType);
...
//                                "by the method of finite differences.");
        ...
        trainGRNN.train(trueTrainingScores);
        ...
        VertexRankingDeepGraph<Void> testGRNN =
                new VertexRankingDeepGraph<>(featureVectorsSize, outputSize, maximumNumberOfSteps, testingRNNGraph, updateFunctionType);
        testGRNN.outputFunctionParameters(trainGRNN.outputFunctionParameters());
        testGRNN.updateFunctionParameters(trainGRNN.updateFunctionParameters());
        ...
        testingRNNGraph.vertices()
                .stream()
                .filter(vertex -> trueTestingScores.containsKey(vertex.content().id()))
                .forEach(vertex -> predictions.put(vertex.content().id(),
                                                   testGRNN.output(vertex)));
    ...
    private void appendEvaluationResultsToFile(int featureVectorsSize,
        ...
        try {
            ...
            Files.write(Paths.get(folderPath + "/results_" + normalizeDataSetName(trainingDataSetName)  + "_" + normalizeDataSetName(testingDataSetName) + "_" + updateFunctionType.name().toLowerCase() + ".txt"),
    ...
    public static void main(String[] args) {
        ...
        VertexRankingExperiment2 experiment = new VertexRankingExperiment2(trainingDataSet,
                                                                           testingDataSet,
                                                                           rankingAlgorithm,
                                                                           UpdateFunctionType.valueOf(args[1]),
                                                                           args[7]);
