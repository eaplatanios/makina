...
public class VertexRankingExperiment {
        ...
        switch (rankingAlgorithm) {
            case PAGERANK:
                ...
                for (Vertex<PageRankAlgorithm.VertexContentType, Void> vertex : prGraph.vertices())
                    trueScores.put(vertex.content().getId(), Vectors.dense((double) vertex.content().getRank()));
                break;
            case HITS:
                ...
                for (Vertex<HITSAlgorithm.VertexContentType, Void> vertex : hitsGraph.vertices())
                    trueScores.put(vertex.content().getId(), Vectors.dense(vertex.content().getAuthorityScore(),
    ...
    private EvaluationResults runRNNExperiment(int featureVectorsSize,
        ...
        VertexRankingDeepGraph<Void> graphRNNAlgorithm =
                new VertexRankingDeepGraph<>(featureVectorsSize, outputSize, maximumNumberOfSteps, rnnGraph, updateFunctionType);
        ...
        rnnGraph.vertices()
                .stream()
                .filter(vertex -> trueScores.containsKey(vertex.content().id()))
                .forEach(vertex -> predictions.put(vertex.content().id(),
                                                   graphRNNAlgorithm.output(vertex)));
