...
public final class ConsensusADMMSolver extends AbstractIterativeSolver {
    ...
    private ConsensusADMMSolver(AbstractBuilder<?> builder) {
        ...
        for (Vector.Element element : variableCopiesCounts) // TODO: Find better way to deal with "useless" variables in the future.
            if (element.value() == 0)
                variableCopiesCounts.set(element.index(), 1);
    ...
    /**
     * TODO: Be smarter in what to update for the stochastic methods. For example, only the residuals of the affected
     * variables need to be recomputed.
     */
    @Override
            ...
            if (penaltyParameterSettingMethod == PenaltyParameterSettingMethod.ADAPTIVE
                ...
                dualResidual = penaltyParameter *
                        Math.sqrt(currentPoint
                                          .sub(previousPoint)
                                          .mapInPlace(x -> x * x)
                                          .multElementwiseInPlace(variableCopiesCounts)
                                          .sum());
            }
        }
