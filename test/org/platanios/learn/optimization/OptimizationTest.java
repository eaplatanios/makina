...
//
    @Test
    public void testCoordinateDescentSolver() {
        System.out.println("Rosenbrock Function Coordinate Descent:\n");
        CoordinateDescentSolver coordinateDescentSolver = new CoordinateDescentSolver(new RosenbrockFunction(), new double[] { -1.2, 1 });
        double[] actualResult = coordinateDescentSolver.solve().toArray();
        double[] expectedResult = new double[] { 1, 1 };
        Assert.assertArrayEquals(expectedResult, actualResult, 0.2);

        System.out.println("Quadratic Function Coordinate Descent:\n");
        RealMatrix A = new Array2DRowRealMatrix(new double[][] { { 1, 0.5 }, { 0.5, 1 } });
        RealVector b = new ArrayRealVector(new double[] { 1, 2 });
